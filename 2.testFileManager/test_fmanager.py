...
import shutil
import pytest
const_dir_path = "/Users/User/PycharmProjects/filemanager/tests"
const_file_path = "/Users/Yangin/PycharmProjects/filemanager/test.txt"

#Кортежи для использования как параметров в тестах
@pytest.mark.parametrize("current, move, symb", [("C:\\Users\\User", "PycharmProjects", "\\"),
                                                 ("C:\\Users\\User\\PycharmProjects", "filemanager", "\\"),
                                                 ("C:\\Users\\User\\PycharmProjects\\filemanager", "tests", "\\")])
def test_move_catalog_positive(current, move, symb):
    fmanager.Cat(current, move, symb)                  #Вызывается функция Cat с параметрами
    assert os.getcwd() == current + symb + move        #assert возвращает true/false в зависимости от того является ли текущая директория = кортежу, т.е. был ли произведен переход из current в move
    print("\n", os.getcwd(), "  =  ", current + symb + move)


@pytest.mark.parametrize("current, move, symb", [("C:\\Users\\User", "NotPycharmProjects", "\\"),
                                                 ("C:\\NotUsers\\User\\PycharmProjects", "filemanager", "\\"),
                                                 ("C:\\Users\\User\\PycharmProjects\\Notfilemanager", "tests", "\\")])
def test_move_catalog_negative(current, move, symb):
    with pytest.raises(FileNotFoundError):         #Проверяет,возникает соответствующая ошибка(несуществующее имя каталога) при вызове Cat
        fmanager.Cat(current, move, symb)
